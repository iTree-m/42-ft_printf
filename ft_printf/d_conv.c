/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   d_conv.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ichejra <ichejra@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/11 15:53:28 by ichejra           #+#    #+#             */
/*   Updated: 2020/01/16 11:54:53 by ichejra          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void	zero_and_dig(const char **s, t_struct *var)
{
	if (var->secondchar == 1)
	{
		print_zero(s, var);
		ft_putnbr(var->d, var);
	}
	else if (var->secondchar == 2)
	{
		if (!var->checkstar)
			print_zero(s, var);
		ft_putnbr(var->d, var);
		if (var->checkstar)
			print_sp(s, var);
	}
	else if (var->secondchar == 3)
	{
		ft_putnbr(var->d, var);
		if (var->thirdchar == 1 || var->thirdchar == 2)
			print_sp(s, var);
	}
	else
		ft_putnbr(var->d, var);
}

static void	min_and_dig(const char **s, t_struct *var)
{
	if (var->secondchar == 1 || (var->secondchar == 2 && var->thirdchar == 1))
	{
		ft_putnbr(var->d, var);
		print_sp(s, var);
	}
	else if ((var->secondchar == 2 && var->thirdchar == 2) ||
			(var->secondchar == 3 && var->thirdchar == 0))
	{
		ft_putnbr(var->d, var);
		print_sp(s, var);
	}
	else
		ft_putnbr(var->d, var);
}

static void	exec_min_star_in_prec(const char **s, t_struct *var)
{
	if (var->secondchar == 3)
	{
		if (var->thirdchar == 1 || var->thirdchar == 2)
		{
			ft_putnbr(var->d, var);
			print_sp(s, var);
		}
		else if (!var->thirdchar)
			if (var->d)
				ft_putnbr(var->d, var);
	}
	else if (var->secondchar == 4)
	{
		if (var->d == 0 && var->star == 0)
			return ;
		if (var->checkstar)
			ft_putnbr(var->d, var);
		else if (!(var->checkstar))
		{
			print_zero(s, var);
			ft_putnbr(var->d, var);
		}
	}
}

static void	prec_and_dig(const char **s, t_struct *var)
{
	if (var->secondchar == 1)
	{
		print_zero(s, var);
		ft_putnbr(var->d, var);
	}
	else if (var->secondchar == 2)
	{
		if (var->thirdchar == 1)
		{
			print_zero(s, var);
			ft_putnbr(var->d, var);
		}
		else if (!var->thirdchar)
			if (var->d)
				ft_putnbr(var->d, var);
	}
	else if (var->secondchar == 3)
		exec_min_star_in_prec(s, var);
	else if (var->secondchar == 4)
		exec_min_star_in_prec(s, var);
	else if (var->d)
		ft_putnbr(var->d, var);
	else
		return ;
}

void		d_conv(const char **s, va_list ap, t_struct *var)
{
	var->d = (int)va_arg(ap, int);
	if (var->firstchar == 1)
	{
		print_sp(s, var);
		ft_putnbr(var->d, var);
	}
	else if (var->firstchar == 2)
		zero_and_dig(s, var);
	else if (var->firstchar == 3)
		min_and_dig(s, var);
	else if (var->firstchar == 4 && var->thirdchar == 0)
	{
		if (var->checkstar)
			ft_putnbr(var->d, var);
		print_sp(s, var);
		if (!var->checkstar)
			ft_putnbr(var->d, var);
	}
	else if (var->firstchar == 5)
		prec_and_dig(s, var);
	else
		ft_putnbr(var->d, var);
}
